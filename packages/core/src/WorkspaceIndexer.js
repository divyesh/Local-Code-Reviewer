...
export class WorkspaceIndexer {
    symbols = [];
    ignoreDirs = ['node_modules', '.git', 'bin', 'obj', '.vs', 'dist', 'vendor', 'target', '.next'];
    supportedExts = ['.ts', '.js', '.py', '.java', '.cs'];
    async indexWorkspace(dirPath) {
        this.symbols = [];
        this.scanDir(dirPath, dirPath);
        return this.symbols;
    }
    scanDir(currentPath, rootPath) {
    ...
    indexFile(filePath, rootPath, ext) {
        try {
