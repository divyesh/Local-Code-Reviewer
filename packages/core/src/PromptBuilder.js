export class PromptBuilder {
    static buildSystemPrompt(workspaceContext) {
...
${diff}`;
    ...
    static buildBestPracticesPrompt(language) {
...
Identify violations that make the code harder to maintain or read.`;
    ...
    static buildLanguageSpecificRules(language) {
        // Basic rules until we implement file-based rule reading
        if (language.toLowerCase() === 'csharp') {
            return `- Use PascalCase for methods/classes.\n- Use _camelCase for private fields.\n- Avoid .Result on Tasks.\n- Prefer string interpolation over concatenation.\n- EF Core: Use .AsNoTracking() for reads, avoid N+1 queries, use .Any() instead of .Count() > 0, and use async DB methods.`;
        }
